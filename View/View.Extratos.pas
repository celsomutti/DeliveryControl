...
type
  ...
  private
    ...
    procedure ProcessaLancamentos(sEntregador, sdata1, sdata2, sExtratos: String);
    procedure LimpaTela;
...
procedure Tview_Extratos.ProcessaLancamentos(sEntregador, sdata1, sdata2, sExtratos: String);
var
...
begin
  try
    ...
    else if (not sdata1.IsEmpty) and (not sdata2.IsEmpty) then
    begin
      if FLancamentos.SearchLancamentosEntregador(sEntregador, sdata1, sdata2) then
      begin
        DM_Main.memTableLancamentos.First;
        i := Pred(stringGridExtrato.RowCount);
        if not DM_Main.memTableLancamentos.IsEmpty then
...
begin
  try
    ...
    begin
      ...
      ProcessaLancamentos(sentregador, sdataini, sdatafim, '');
      if dTotal > 0 then
      begin
        stringGridExtrato.RowCount := (stringGridExtrato.RowCount + 1);
        i := Pred(stringGridExtrato.RowCount);
