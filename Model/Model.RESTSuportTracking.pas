...
procedure TRESTSuportTracking.StartRestClient(sFile: String);
begin
...
procedure TRESTSuportTracking.StartRestRequest;
begin
  StartRestClient('/dc_suport_tracking.php');
...
function TRESTSuportTracking.SearchTracking(sValue: String): Boolean;
begin
  Result := False;
  StartRestRequest;
  DM_Main.RESTResponseDataSetAdapter.Dataset := DM_Main.memTableTracking;
  DM_Main.RESTRequest.AddParameter('remessa', sValue, pkGETorPOST);
  DM_Main.RESTRequest.AddParameter('awb', sValue, pkGETorPOST);
  DM_Main.RESTResponseDataSetAdapter.Dataset := DM_Main.memTableTracking;
  DM_Main.RESTResponseDataSetAdapter.Active := True;
  DM_Main.RESTRequest.Execute;
  if DM_Main.memTableTracking.IsEmpty then Exit;
  Result := True;
end;
